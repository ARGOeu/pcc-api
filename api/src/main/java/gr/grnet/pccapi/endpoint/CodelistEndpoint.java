...
public class CodelistEndpoint {
  ...
  public Response getByCategory(
      @Parameter(
              ...
              required = false,
              examples = {
                @ExampleObject(name = "all elements"),
                @ExampleObject(
                    name = "lookup_service_type elements",
                    value = "lookup_service_type"),
                @ExampleObject(name = "contract_type elements", value = "contract_type")
              },
              description =
                  "Indicates the category of the codelist elements to return. if empty returns all elements")
          @QueryParam("category")
          String category) {
