...
public class EOSCPortalTest {
  ...
  public void updateEoscPortalDomains() throws ExecutionException, InterruptedException {
    ...
    assertEquals(
        DomainMapper.INSTANCE.eoscPortalDomainToDomain(existPortalDomain),
        domainRepository.findByDomainId("scientific_domain-social_sciences").get());
    assertEquals(
        DomainMapper.INSTANCE.eoscPortalDomainToDomain(eoscPortalDomain),
        domainRepository.findByDomainId("scientific_domain-medical_and_health_sciences").get());
  }
}
